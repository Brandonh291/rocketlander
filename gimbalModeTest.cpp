...
double seaLevelhPa = 1019.1; //in mb
...
float gx, gy, gz;
float mx, my, mz;
float gx_unfil, gy_unfil, gz_unfil;
...
double RELEASE_HEIGHT = 0.25; //19; //meters
...
int main(){
    ...
    while(notLanded){ 
        ...
        gy_unfil = gy-gbias[1];
        gz_unfil = gz-gbias[2];
        //printf("unfiltered data: x %f y: %f z: %f \n", gx_unfil, gy_unfil, gz_unfil);
        if(startup=false){
            //filter();
        }
        data +=1;
        ...
        switch (mode){            
            ...
            case MODE_IGNITE:
                //pwm->set_duty_cycle(ledPin, 40000);//ignite the motor by sending a high current through the ignition circuit
            ...
            case MODE_DECEL:
                ...
                if(blinker >= 1000) mode=MODE_LAND;
                break;
...
void saveData(){
    ...
    myfile << curTime << "," << q[0] << "," << q[1] << "," << q[2] << "," << q[3] << "," << gx << "," << gy << "," << gz << ","
           << ax << "," << ay << "," << az << "," << mx << "," << my << "," << mz << "," << gx_unfil << "," << gy_unfil << "," 
           << gz_unfil << "," <<  alt << "," << height << "," << "," << press << "," << accel << "," << mode << "\n";            
    myfile.close();
 }
